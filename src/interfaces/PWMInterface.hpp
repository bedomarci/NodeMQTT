...
public:
    ...
    void fadeTo(uint16_t fadeTo, uint16_t duration, uint16_t delayed = 0);
...
inline void PWMInterface::fadeTo(uint16_t fadeTo, uint16_t duration, uint16_t delayed) {
    this->fade(this->read(), fadeTo, duration, delayed);
}
