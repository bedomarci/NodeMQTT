...
inline void DFPlayerInterface::init() {
    if (DFSerial == nullptr) {
#ifdef ESP8266
        SoftwareSerial *serial = new SoftwareSerial(_serialRxPin, _serialTxPin);
        serial->listen();
#endif
#ifdef ESP32
...
inline void DFPlayerInterface::printDetail(uint8_t type, int value) {
    ...
    switch (type) {
        ...
        case DFPlayerCardInserted:
            ...
            Logger.logf(L_INFO, buffer, this->readFileCounts());
            break;
...
void DFPlayerInterface::setSerial(Stream *serial) {
    this->DFSerial = serial;
}
